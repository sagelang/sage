...
pub enum CodegenTarget {
    /// Native binary (uses tokio, signal handlers, etc.)
    #[default]
    Native,
    /// WebAssembly (uses wasm-bindgen, spawn_local, no signals)
    Wasm,
}
...
impl Generator {
    ...
    fn generate_program(&mut self, program: &Program) -> String {
        ...
        if self.config.target == CodegenTarget::Wasm {
            self.emit.writeln("use wasm_bindgen::prelude::*;");
        }
        self.emit.blank_line();
...
"#
    ...
    fn generate_wasm_cargo_toml(&self, name: &str) -> String {
        let runtime_dep = self.config.runtime_dep.to_cargo_dep(None);

        // Derive sage-runtime-web path from runtime dep
        let web_dep = match &self.config.runtime_dep {
            RuntimeDep::Path { path } => {
                // Sibling crate: replace sage-runtime with sage-runtime-web
                let web_path = path.replace("sage-runtime", "sage-runtime-web");
                format!("sage-runtime-web = {{ path = \"{}\" }}", web_path)
            }
            RuntimeDep::CratesIo { version } => {
                format!("sage-runtime-web = \"{version}\"")
            }
        };
...
"#
    ...
    fn generate_main(&mut self, agent: &AgentDecl) {
        if self.config.target == CodegenTarget::Wasm {
            return self.generate_wasm_main(agent);
        }
    ...
    /// Generate WASM entry point using wasm_bindgen(start).
    ///
    /// Instead of `#[tokio::main] async fn main()`, generates a `#[wasm_bindgen(start)]`
    /// function that sets up panic hook, tracing, and spawns the agent via spawn_local.
    fn generate_wasm_main(&mut self, agent: &AgentDecl) {
        let entry_agent = &agent.name.name;

        // Set current protocol roles (same as native)
        self.current_protocol_roles.clear();
        for pr in &agent.follows {
            self.current_protocol_roles
                .insert(pr.protocol.name.clone(), pr.role.name.clone());
        ...
        // Set current agent's @persistent beliefs
        self.current_agent_persistent_beliefs = agent
            .beliefs
            .iter()
            .filter(|b| b.is_persistent)
        ...
        // WASM entry point
        self.emit.writeln("#[wasm_bindgen(start)]");
        self.emit.writeln("pub fn wasm_main() {");
        self.emit.indent();

        // Panic hook for browser console
        self.emit.writeln("console_error_panic_hook::set_once();");
        self.emit.writeln("");

        // Tracing init (console backend on WASM)
        self.emit.writeln("sage_runtime::trace::init();");
        self.emit.writeln("");
        ...
        if has_persistent {
            self.emit
                .writeln("// Initialize persistence checkpoint store (memory-only on WASM)");
            self.emit.writeln(
                "let _checkpoint: std::sync::Arc<dyn CheckpointStore> = std::sync::Arc::new(",
            );
            self.emit.indent();
        ...
        // Build the agent construction expression (same logic as native)
        let needs_struct = has_tools || !agent.beliefs.is_empty();
        let agent_construct = if needs_struct {
            let mut s = format!("{entry_agent} {{ ");
            let mut fields = Vec::new();
        ...
        // Spawn the agent via spawn_local (no Send bounds on WASM)
        self.emit
            .writeln("let handle = sage_runtime::spawn(move |mut ctx| async move {");
        self.emit.indent();

        // Async tool initialization inside spawn closure
        for tool_use in &agent.tool_uses {
            if tool_use.name == "Database" {
                self.emit
                    .write("let _db_client = DatabaseClient::from_env().await");
        ...
        // Protocol roles for message dispatch
        let protocol_roles: Vec<_> = agent
            .follows
            .iter()
            .map(|pr| (pr.protocol.name.clone(), pr.role.name.clone()))
        ...
        // Message receive loop (same as native)
        if has_message_handlers {
            self.emit.writeln("");
            self.emit.writeln("// Message receive loop");
            self.emit.writeln("if result.is_ok() {");
            ...
            for (param_name, param_ty) in &message_handlers {
                ...
                if has_protocols {
                    if let Some((_, role)) = protocol_roles.first() {
                        self.emit.writeln("// Validate protocol state");
                        self.emit.write("ctx.validate_protocol_receive(\"");
                        self.emit.write(&type_name);
                        self.emit.write("\", \"");
                        self.emit.write(role);
        ...
        // On WASM, we spawn_local to await the handle result and log errors
        self.emit.writeln("wasm_bindgen_futures::spawn_local(async move {");
        self.emit.indent();
        self.emit.writeln("match handle.result().await {");
        self.emit.indent();
        self.emit.writeln("Ok(_) => sage_runtime_web::console_log(\"Agent completed successfully\"),");
        self.emit.writeln("Err(e) => sage_runtime_web::console_error(&format!(\"Agent failed: {e}\")),");
        self.emit.dedent();
        self.emit.writeln("}");
        self.emit.dedent();
        self.emit.writeln("});");
...
"#;
    ...
    // =========================================================================
    // WASM target codegen tests
    // =========================================================================

    fn generate_wasm_source(source: &str) -> String {
        let lex_result = lex(source).expect("lexing failed");
        let source_arc: Arc<str> = Arc::from(source);
        let (program, errors) = parse(lex_result.tokens(), source_arc);
        assert!(errors.is_empty(), "parse errors: {errors:?}");
    ...
    fn generate_wasm_cargo_toml(source: &str) -> String {
        let lex_result = lex(source).expect("lexing failed");
        let source_arc: Arc<str> = Arc::from(source);
        let (program, errors) = parse(lex_result.tokens(), source_arc);
        assert!(errors.is_empty(), "parse errors: {errors:?}");
        ...
        generate_with_full_config(&program, "test", config).cargo_toml
    }

    #[test]
    fn generate_wasm_entry_point() {
        let source = r#"
            agent Main {
                on start {
                    print("hello wasm");
        ...
        let output = generate_wasm_source(source);
        assert!(
            output.contains("#[wasm_bindgen(start)]"),
            "Should have wasm_bindgen(start) entry point"
        );
        assert!(
            output.contains("console_error_panic_hook::set_once()"),
            "Should set panic hook"
        );
        assert!(
            output.contains("sage_runtime::trace::init()"),
            "Should init tracing"
        );
        assert!(
            output.contains("sage_runtime::spawn("),
            "Should spawn agent"
        );
        assert!(
            output.contains("wasm_bindgen_futures::spawn_local("),
            "Should spawn_local to await result"
        );
        assert!(
            !output.contains("tokio::main"),
            "Should NOT have tokio::main"
        );
        assert!(
            !output.contains("ctrl_c"),
            "Should NOT have signal handlers"
        );
    }

    #[test]
    fn generate_wasm_imports() {
        let source = r#"
            agent Main {
                on start { yield(0); }
            }
        ...
        let output = generate_wasm_source(source);
        assert!(
            output.contains("use wasm_bindgen::prelude::*;"),
            "Should import wasm_bindgen prelude"
        );
    }

    #[test]
    fn generate_wasm_cargo_toml_cdylib() {
        let source = r#"
            agent Main {
                on start { yield(0); }
            }
        ...
        let toml = generate_wasm_cargo_toml(source);
        assert!(
        ...
        assert!(
            ...
            "Should depend on wasm-bindgen"
        );
        assert!(
            toml.contains("console_error_panic_hook"),
            "Should depend on console_error_panic_hook"
        );
        assert!(
            !toml.contains("tokio"),
            "Should NOT depend on tokio"
        );
        assert!(
            toml.contains("wasm-release"),
            "Should have wasm-release profile"
        );
    }

    #[test]
    fn generate_wasm_with_tools() {
        let source = r#"
            agent Main {
                use Http
        ...
        let output = generate_wasm_source(source);
        assert!(
            output.contains("HttpClient::from_env()"),
            "Should construct HTTP tool"
        );
        assert!(
            output.contains("wasm_bindgen(start)"),
            "Should still have WASM entry point"
        );
    }

    #[test]
    fn generate_wasm_with_persistent() {
        let source = r#"
            agent Main {
                @persistent count: Int
        ...
        assert!(
            output.contains("MemoryCheckpointStore::new()"),
            "Should use memory checkpoint store on WASM: {output}"
        );
        assert!(
            output.contains("Persisted::new("),
